...
import { loginThunk } from "../features/asyncThunks";
...
export default function Login() {
  ...
  const { handleSubmit, getFieldProps, touched, errors, isSubmitting, isValid } = useFormik({
    ...
    async onSubmit(data, { setSubmitting }) {
      try {
        setSubmitting(true);
        const resp = await dispatch(loginThunk({ email: data.email, password: data.password }));
        unwrapResult(resp);
        navigate("/home");
        setSubmitting(false);
      } catch (error: any) {
  ...
  return (
    <Box display="flex" justifyContent="center" alignItems="center" flexDirection="column" mt={10}>
      ...
      <Link sx={{ textDecoration: "none" }} to="/register">
